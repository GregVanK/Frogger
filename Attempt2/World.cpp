...
namespace GEX {
	...
	void World::addEnemies()
	{
		...
		addEnemy(AircraftType::Avenger, -100.f, 1300.f);
		addEnemy(AircraftType::Avenger, -200.f, 1300.f);
		addEnemy(AircraftType::Boss, 0.f, 500.f);
	...
	/*void World::bossEnteredView()
	{
		Command command;
		command.category = Category::Type::Boss;
		command.action = derivedAction<Aircraft>([this](Aircraft& e, sf::Time dt) {
			if (getBattlefieldBounds().intersects(e.getBoundingBox())) {
				e.updateBossMovement();
			}
	...
	void World::loadTextures()
	{
		...
		_textures.load(GEX::TextureID::Boss, "Media/Textures/Eagle.png");
	...
	void World::handleCollisions()
	{
		...
		for (SceneNode::Pair pair : collisionPairs) {
			if (matchesCategories(pair, Category::Type::PlayerAircraft, Category::Type::EnemyAircraft)) {
				...
				pickup.apply(player);
				pickup.destroy();
				player.playLocalSound(_command, SoundEffectID::CollectPickup);
			}
